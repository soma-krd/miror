...
interface MirState {
  ...
  // mir.json import/export
  exportMirJson: (projectId: string) => Promise<void>;
  importMirJson: (projectId: string) => Promise<void>;
...
export const useMir = create<MirState>((set, get) => ({
  projects: [],
  services: [],
  logs: [],
  ...
  exportMirJson: async (projectId) => {
    try {
      await backend.exportMirJson(projectId);
      alert("mir.json exported to project root ✓");
    } catch (e) {
      console.error("exportMirJson failed:", e);
      alert(`Failed to export: ${e instanceof Error ? e.message : String(e)}`);
    }
  },

  importMirJson: async (projectId) => {
    try {
      const result = await backend.importMirJson(projectId);
      alert(`Imported ${result.imported} services from mir.json ✓`);
      await get().refreshServices();
    } catch (e) {
      console.error("importMirJson failed:", e);
...
function fireNativeNotification(n: BackendNotification) {
  ...
  const title = `Mir — ${n.service_name}`;
