      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-border/50">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={cn("flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] font-medium transition-all miror-press",
        ...
        <div className="ml-auto flex items-center gap-2">
          <Button variant="ghost" size="sm" className="h-7 gap-1 text-[11px] miror-press"
        ...
        {!loading && tab === "dashboard" && stats && (
          <div className="p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Card className="miror-card border-primary/20">
                <CardContent className="p-4">
...
function BigCounter({ icon: Icon, label, value, color, subtext }: { icon: typeof Keyboard; label: string; value: string; color: string; subtext: string }) {
  return (
    <Card className="miror-card overflow-hidden">
