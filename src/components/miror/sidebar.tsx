...
import { useMiror, type ViewId } from "@/store/miror-store";
import { StatusDot } from "./status-badge";
import { MirorLogo } from "./miror-logo";
...
export function Sidebar() {
  ...
  if (sidebarCollapsed) {
    return (
      <aside className="miror-sidebar flex w-14 flex-col items-center gap-1.5 py-4">
        <div className="mb-2">
          <MirorLogo className="h-7 w-7 shrink-0" />
        </div>
        <Button
          variant="ghost"
          size="icon"
          ...
          className="h-8 w-8 text-sidebar-foreground/70 hover:text-sidebar-foreground miror-press"
        >
  ...
  return (
    <aside className="miror-sidebar flex w-60 flex-col">
      ...
      <div className="flex items-center gap-2.5 px-4 pt-4 pb-3">
        ...
        <Button
          ...
          className="h-6 w-6 text-sidebar-foreground/50 hover:text-sidebar-foreground miror-press"
        >
      ...
      <nav className="px-2.5 pb-1.5">
        {NAV.map((item) => (
          <button
            ...
            className={cn(
              "group flex w-full items-center gap-2.5 rounded-lg px-2.5 py-[6px] text-[13px] font-medium transition-all miror-press",
              view === item.id
                ? "bg-sidebar-accent text-sidebar-accent-foreground"
      ...
      {/* Projects section */}
      <div className="flex-1 overflow-y-auto miror-scroll px-2.5 pb-2">
                  ...
                  {projServices.map((svc) => (
                    <button
                      ...
                      onClick={() => {
                        setActiveProject(proj.id);
                        setView("logs");
                        useMiror.getState().setActiveService(svc.id);
                      }}
