...
// with Miror's running services, and surfaces conflicts (2+ processes on same port).
// One-click kill for orphan processes holding ports Miror services need.
...
export function PortRadar({ open, onOpenChange }: PortRadarProps) {
  ...
  const [filter, setFilter] = useState<"all" | "miror" | "system" | "conflicts">("all");
  ...
  const filteredMappings = (scan?.mappings ?? []).filter(m => {
    if (filter === "miror") return m.serviceId;
    if (filter === "system") return !m.serviceId;
    if (filter === "conflicts") return scan?.conflicts.some(c => c.port === m.port);
    return true;
  });

  const mirorCount = scan?.mappings.filter(m => m.serviceId).length ?? 0;
  const systemCount = scan?.mappings.length ?? 0 - mirorCount;
      ...
      <DialogContent className="sm:max-w-3xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex items-center gap-2 border-b border-border pb-2">
          <div className="flex items-center gap-1">
            {(["all", "miror", "system", "conflicts"] as const).map(f => {
        ...
        {/* Port list */}
        <div className="flex-1 overflow-y-auto miror-scroll -mx-2 px-2">
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {killTarget?.name && scan?.mappings.find(m => m.pid === killTarget.pid)?.serviceId && (
                <span className="block mt-2 text-status-error">⚠ This is a Miror-managed service. Use the Stop button instead for graceful shutdown.</span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
