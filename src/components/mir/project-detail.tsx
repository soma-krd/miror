...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  const {
    ...
    openInEditor, exportMirJson, importMirJson,
  } = useMir();
  ...
  return (
    <div className="space-y-4 p-4">
      ...
      {/* Project Tools — Docker, mir.json, env profile, editor integration */}
          ...
          <CardContent className="text-xs space-y-2">
            ...
            <div className="flex gap-1.5">
              <Button
                ...
                onClick={() => exportMirJson(projectId)}
              >
                <RefreshCw className="h-3 w-3" /> Export
              </Button>
              <Button
                variant="outline" size="sm" className="h-7 gap-1 text-[11px] flex-1"
                onClick={() => importMirJson(projectId)}
              >
...
function DependencyGraph({
  ...
  return (
    <div className="flex items-stretch gap-6 overflow-x-auto mir-scroll pb-2">
